...
def getOutputPath(trial,participant,session,part):
    # print ""
    ...
    outfile = str(trial) + '_' + str(participant) + '_' + str(session) + '_' + str(part)+'_ACC'
    outputPath = outfile

    fullOutputPath = outfile
...
def loadData_getColNames(data_columns):
    ...
    # Find the column names for each of the 4 data streams
    colnames = ['Time', 'Acceleration X', 'Acceleration Y', 'Acceleration Z']
    new_colnames = ['time', 'x', 'y', 'z']
...
def loadData_misc(filepath):
    ...
    data.rename(columns=dict(zip(new_colnames, ['time', 'AccelX', 'AccelY', 'AccelZ'])), inplace=True)
    data = data[['AccelZ', 'AccelY', 'AccelX', 'time']]
